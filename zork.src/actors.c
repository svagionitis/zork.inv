...
L1400:
    if (prsvec_1.prsa == vindex_1.walkw || prsvec_1.prsa == vindex_1.takew ||
        ...
        prsvec_1.prsa == vindex_1.pushw || prsvec_1.prsa == vindex_1.throww ||
        prsvec_1.prsa == vindex_1.turnw || prsvec_1.prsa == vindex_1.leapw)
        goto L10;
...
/* 						!WALK? */
    i = 784;
/* 						!ASSUME WONT. */
    if ((play_1.here == rindex_1.scorr &&
        (prsvec_1.prso == xsrch_1.xnorth || prsvec_1.prso == xsrch_1.xenter)) ||
        (play_1.here == rindex_1.ncorr &&
        (prsvec_1.prso == xsrch_1.xsouth || prsvec_1.prso == xsrch_1.xenter)))
        i = 785;
...
L2200:
    if (prsvec_1.prsa == vindex_1.takew || prsvec_1.prsa == vindex_1.dropw ||
        prsvec_1.prsa == vindex_1.putw || prsvec_1.prsa == vindex_1.throww ||
        prsvec_1.prsa == vindex_1.pushw || prsvec_1.prsa == vindex_1.turnw ||
        prsvec_1.prsa == vindex_1.spinw || prsvec_1.prsa == vindex_1.trntow ||
        prsvec_1.prsa == vindex_1.follow || prsvec_1.prsa == vindex_1.stayw ||
        prsvec_1.prsa == vindex_1.openw || prsvec_1.prsa == vindex_1.closew ||
        prsvec_1.prsa == vindex_1.killw)
        goto L10;
...
/* 						!YES, VANISH. */
    rhere = 0;
    if (qhere_(oindex_1.still, rindex_1.treas) ||
        objcts_1.oadv[oindex_1.still - 1] == -oindex_1.thief)
...
/* 						!70% CHANCE TO DO NOTHING. */
    ...
    i__1 = -oindex_1.thief;
    i__2 = -oindex_1.thief;
    nr = robrm_(hack_1.thfpos, 100, 0, 0, i__1) +
         robadv_(play_1.winner, 0, 0, i__2);
    i = 586;
/* 						!ROBBED EM. */
    if (rhere != 0)
        i = 588;
...
/* 						!BOTH IN MAZE. */
        if (! qhere_(i, hack_1.thfpos) || prob_(60, 60) ||
            (objcts_1.oflag1[i - 1] & (VISIBT + TAKEBT)) != VISIBT + TAKEBT)
            goto L1450;
...
L1500:
    ...
    for (i = 1; i <= i__1; ++i) {
/* 						!NOT IN MAZE. */
        if (! qhere_(i, hack_1.thfpos) || objcts_1.otval[i - 1] != 0 || prob_(80, 60) ||
            (objcts_1.oflag1[i - 1] & (VISIBT + TAKEBT)) != VISIBT + TAKEBT)
            goto L1550;
...
/* 						!NO, DROP STUFF. */
    ...
    for (i = 1; i <= i__1; ++i) {
        if (objcts_1.oadv[i - 1] != -oindex_1.thief ||
            prob_(70, 70) || objcts_1.otval[i - 1] > 0)
            goto L1850;
